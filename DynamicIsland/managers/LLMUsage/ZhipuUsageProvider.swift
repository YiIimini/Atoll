...
/// Tracks 智谱 GLM API usage via Atoll's Screen Assistant call log.
struct ZhipuUsageProvider: UsageProvider {
    ...
    func fetchSnapshot(now: Date) async throws -> UsageSnapshot {
        guard FileManager.default.fileExists(atPath: logDir.path) else {
            throw UsageError.notFound("智谱 GLM 用量日志尚未生成 — 请在屏幕助手中使用后重试")
        }
        let files = jsonlFiles(under: logDir).filter { $0.lastPathComponent.contains("zhipu") }
        guard !files.isEmpty else {
            throw UsageError.notFound("未找到 智谱 GLM 用量记录")
        }
