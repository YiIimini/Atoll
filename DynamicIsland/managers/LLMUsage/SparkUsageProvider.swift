...
/// Tracks 讯飞星火 API usage via Atoll's Screen Assistant call log.
struct SparkUsageProvider: UsageProvider {
    let id: ProviderID = .spark
    let logDir: URL
    ...
    func fetchSnapshot(now: Date) async throws -> UsageSnapshot {
        guard FileManager.default.fileExists(atPath: logDir.path) else {
            throw UsageError.notFound("讯飞星火 用量日志尚未生成 — 请在屏幕助手中使用后重试")
        }
        let files = jsonlFiles(under: logDir).filter { $0.lastPathComponent.contains("spark") }
        guard !files.isEmpty else {
            throw UsageError.notFound("未找到 讯飞星火 用量记录")
        }
