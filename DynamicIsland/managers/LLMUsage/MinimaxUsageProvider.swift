...
/// Tracks Minimax API usage via Atoll's Screen Assistant call log.
struct MinimaxUsageProvider: UsageProvider {
    ...
    func fetchSnapshot(now: Date) async throws -> UsageSnapshot {
        guard FileManager.default.fileExists(atPath: logDir.path) else {
            throw UsageError.notFound("Minimax 用量日志尚未生成 — 请在屏幕助手中使用后重试")
        }
        let files = jsonlFiles(under: logDir).filter { $0.lastPathComponent.contains("minimax") }
        guard !files.isEmpty else {
            throw UsageError.notFound("未找到 Minimax 用量记录")
        }
